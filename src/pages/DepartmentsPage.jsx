...
export default function DepartmentsPage() {
    ...
    const columns = [
        ...
        },
        {
            key: 'status',
            header: 'Trạng thái',
            render: (dept) => (<Badge variant={dept.status === 'active' ? 'default' : 'secondary'} className={dept.status === 'active' ? 'bg-status-completed' : 'bg-muted'}>
        ...
        },
        {
            ...
            header: '',
            className: 'w-32',
            render: (dept) => (<div className="flex gap-1">
          <Button variant="ghost" size="sm">
            <Edit className="w-4 h-4"/>
          </Button>
          <Button variant="ghost" size="sm">
            {dept.status === 'active' ? (<Lock className="w-4 h-4 text-status-overdue"/>) : (<Unlock className="w-4 h-4 text-status-completed"/>)}
          </Button>
        </div>),
        },
    ];
    return (<div>
      <PageHeader title="Quản lý Phòng ban" description="Quản lý các phòng ban và bộ môn trong trung tâm" actions={<Button onClick={() => setShowCreateDialog(true)}>
            <Plus className="w-4 h-4 mr-2"/>
            Thêm phòng ban
          </Button>}/>

      <DataTable data={mockDepartments} columns={columns} keyExtractor={(dept) => dept.id} emptyMessage="Không có phòng ban nào"/>
