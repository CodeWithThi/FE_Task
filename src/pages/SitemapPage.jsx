...
const sitemapData = [
    ...
    },
    {
        ...
        icon: Users,
        pages: [
            { name: 'Quản lý người dùng', path: '/members', description: 'Danh sách người dùng', roles: ['admin', 'pmo'] },
    ...
    },
    {
        ...
        icon: Settings,
        pages: [
            { name: 'Cài đặt', path: '/system/settings', description: 'Cấu hình hệ thống', roles: ['admin'] },
            { name: 'Nhật ký', path: '/system/logs', description: 'Log hoạt động', roles: ['admin'] },
        ]
    },
];
const workflowSteps = [
      ...
      <div className="container mx-auto px-6 py-8">
        ...
        <section className="mb-12">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
            {sitemapData.map((section, idx) => (<Card key={idx} className="card-hover">
                ...
                <CardContent className="space-y-2">
                  {section.pages.map((page, pidx) => (<div key={pidx} className="p-2 rounded-lg bg-muted/50 hover:bg-muted transition-colors">
                      <p className="font-medium text-sm text-foreground">{page.name}</p>
                      <p className="text-xs text-muted-foreground">{page.description}</p>
                      {page.roles && (<div className="flex gap-1 mt-1 flex-wrap">
                          {page.roles.map(role => (<span key={role} className="text-[10px] px-1.5 py-0.5 bg-primary/10 text-primary rounded">
                              {role}
                            </span>))}
                        </div>)}
                    </div>))}
          ...
          <Card>
            <CardContent className="p-6">
              <div className="flex flex-wrap items-start gap-4">
                {workflowSteps.map((step, idx) => (<div key={idx} className="flex items-center">
                    <div className="flex flex-col items-center min-w-[140px]">
                      ...
                      <div className="text-left w-full">
                        {step.actions.map((action, aidx) => (<p key={aidx} className="text-xs text-muted-foreground">• {action}</p>))}
                      </div>
                    </div>
                    {idx < workflowSteps.length - 1 && (<ArrowRight className="w-6 h-6 text-muted-foreground mx-2 mt-[-60px]"/>)}
                  </div>))}
              </div>
            </CardContent>
          </Card>
        ...
        <section>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-5 gap-4">
            {rolePermissions.map((roleData, idx) => (<Card key={idx} className="card-hover">
                <CardHeader className="pb-3">
                ...
                <CardContent className="space-y-2">
                  {roleData.permissions.map((perm, pidx) => (<div key={pidx} className="flex items-center gap-2 text-sm">
                      {perm.allowed ? (<CheckCircle className="w-4 h-4 text-green-500"/>) : (<XCircle className="w-4 h-4 text-red-400"/>)}
