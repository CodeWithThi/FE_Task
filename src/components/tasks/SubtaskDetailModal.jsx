...
export function SubtaskDetailModal({ open, onOpenChange, task }) {
    const { user } = useAuth();
    ...
    const handleAssigneeChange = (staff) => {
        ...
        <div className="flex flex-col md:flex-row max-h-[calc(90vh-100px)] overflow-hidden">
          ...
          <div className="flex-1 p-6 overflow-y-auto space-y-6 border-r">
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                ...
                {canEdit && (<Button size="sm" variant="outline">
                    <Plus className="w-3 h-3 mr-1"/>
                    Thêm
                  </Button>)}
              </div>
              <div className="space-y-2">
                {checklist.map((item) => (<div key={item.id} className="flex items-center gap-3 p-2 rounded hover:bg-muted/50">
                    <Checkbox checked={item.completed} onCheckedChange={() => canEdit && toggleChecklistItem(item.id)} disabled={!canEdit}/>
            ...
            <div>
              ...
              <div className="space-y-2">
                {mockAttachments.map((file) => (<div key={file.id} className="flex items-center gap-3 p-2 rounded-lg border hover:bg-muted/50 group">
                    <FileText className="w-5 h-5 text-primary"/>
                    <div className="flex-1">
                      <p className="text-sm font-medium">{file.name}</p>
                      <p className="text-xs text-muted-foreground">{file.size}</p>
                    </div>
                    {canEdit && (<Button size="icon" variant="ghost" className="h-8 w-8 opacity-0 group-hover:opacity-100">
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                ...
                {canEdit && (<Button size="sm" variant="outline" onClick={() => setShowAddLink(!showAddLink)}>
                    <Plus className="w-3 h-3 mr-1"/>
                    Thêm
                  </Button>)}
              </div>

              {showAddLink && (<div className="space-y-2 mb-3 p-3 bg-muted/50 rounded-lg">
              ...
              <div className="space-y-2">
                {mockLinks.map((link) => (<a key={link.id} href={link.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 p-2 rounded-lg border hover:bg-muted/50 group">
            ...
            <div>
              ...
              <div className="space-y-3">
                {mockActivityLog.map((log) => (<div key={log.id} className="flex gap-3 text-sm">
          ...
          <div className="w-full md:w-72 p-6 bg-muted/30 space-y-4">
            ...
            <div className="relative">
              ...
              {showAssigneeDropdown && canAssign && (<div className="absolute top-full left-0 right-0 mt-1 bg-card border rounded-lg shadow-lg z-10 overflow-hidden">
                  <div className="p-2 text-xs text-muted-foreground border-b">Chọn người thực hiện</div>
                  {mockStaffList.map((staff) => (<div key={staff.id} onClick={() => handleAssigneeChange(staff)} className="flex items-center gap-2 p-2 hover:bg-muted cursor-pointer">
                      <Avatar className="w-6 h-6">
                        <AvatarFallback className="text-xs bg-primary/10 text-primary">
                          {staff.name.charAt(0)}
                        </AvatarFallback>
